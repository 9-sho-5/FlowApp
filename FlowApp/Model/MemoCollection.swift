...
class MemoCollection: NSObject {
    ...
    class func convertDictionary(memo: Memo) -> Dictionary<String, AnyObject> {
        ...
        dic["detail"] = memo.detail as AnyObject
    ...
    class func convertMemoModel(attiributes: Dictionary<String, AnyObject>) -> Memo {
        let memo = Memo()
        memo.text = attiributes["text"] as! String
        memo.detail = attiributes["detail"] as! String
    ...
    func fetchTodos() {
        let defaults = UserDefaults.standard
        if let memoList = defaults.object(forKey: "memoLists") as? Array<Dictionary<String, AnyObject>> {
